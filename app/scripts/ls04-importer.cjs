#!/usr/bin/env node
/**
 * Importateur du Loup Solitaire 04 — Le Gouffre Maudit (350 paragraphes).
 *
 *   node scripts/ls04-importer.cjs            # génère sections + textes
 *   node scripts/ls04-importer.cjs --audit    # audit seul, n'écrit rien
 *   node scripts/ls04-importer.cjs --carte    # carte compacte des 350 § (relecture)
 *
 * Source : content/stories/ls04-texte-brut.txt (extraction PyMuPDF du PDF,
 ...
 * puis applique les correctifs éditoriaux de ls04-overrides.json :
 *   - app/content/lonewolf/ls04/textes.json
 *   - app/content/lonewolf/ls04/sections-XXX.ts
...
const brutPath = path.join(depot, "content", "stories", "ls04-texte-brut.txt");
const overridesPath = path.join(__dirname, "ls04-overrides.json");
const sortieDir = path.join(racine, "content", "lonewolf", "ls04");
...
const CORRECTIONS = [
  ...
  // Coquilles spécifiques LS04
  [/e§t invulnérable/g, "est invulnérable"],
  [/Rendezvous/g, "Rendez-vous"],
  [/rendezvous/g, "rendez-vous"],
  [/Rende[z]?-?vous/g, "Rendez-vous"],
];
...
/*    Pour LS04, le tableau des rangs Kaï (page 21) contient des      */
...
for (let i = 0; i < lignes.length; i++) {
  if (lignes[i].includes("Pendant trois jours")) {
...
const RE_CIBLE =
  /(?:rend(?:ez[- ]vous)?|vous rendrez|en vous rendant|vous rendre)(?:[a-zà-ÿéèêàçûôî'’ ]{0,30})au\s+(\d{1,3})/i;
...
function serialiserEnnemi(c) {
  ...
  // Champs spécifiques LS04 (conservés pour audit, même si non typés)
